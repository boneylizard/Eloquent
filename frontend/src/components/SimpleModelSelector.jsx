...
const ModelSelector = () => {
  ...
  const isAPIModel = (modelId) => {
    return API_MODELS.some(api => api.id === modelId);
  };

  // Get the API info for a model
  const getAPIInfo = (modelId) => {
    return API_MODELS.find(api => api.id === modelId);
  };
  ...
  const handleLoadModel = async () => {
    ...
    if (isAPIModel(selectedModel)) {
      ...
      if (isAPIModel(selectedModel)) {
  // Just set state - don't call loadModel
  if (gpuId === 0) {
    setPrimaryIsAPI(true);
    setPrimaryModel(selectedModel);
  }
  // DON'T call loadModel for API endpoints!
} else {
  // Only call loadModel for actual GGUF files
