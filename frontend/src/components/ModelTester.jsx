...
Respond with ONLY the score and reason.`;
  ...
  const exportResults = useCallback(() => {
    ...
    const categoryAnalysis = Object.keys(categoryStats).map(category => {
      const totalScore = categoryStats[category].totalScore;
    ...
    const exportData = {
      exportInfo: {
        ...
        overallAverageScore: testResults.length > 0 ?
          (testResults.filter(r => typeof r.score === 'number').reduce((sum, r) => sum + r.score, 0) /
            testResults.filter(r => typeof r.score === 'number').length || 0).toFixed(1) : "0.0",
        eloRatings, // Added ELO Ratings to export
      ...
      if ((judgeMode === 'automated' || judgeMode === 'both') && judgeModelName) {
        ...
        if (judgment) {
          const result = {
            ...
            judgeType: 'automated',
            timestamp: new Date().toISOString(),
            // Assign numeric score for aggregation (Win=100, Tie=50, Loss=0)
            score: judgment.verdict === 'A' ? 100 : judgment.verdict === 'B' ? 0 : 50
          };
          setTestResults(prev => [...prev, result]);
